...
@section('content')
    <section class="mx-auto w-full max-w-3xl rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <form method="POST" action="{{ route('admin.products.store') }}" class="mt-6 grid gap-4 md:grid-cols-2">
            @csrf
            ...
            <div class="md:col-span-2">
                ...
                <input id="image_url" name="image_url" type="url" value="{{ old('image_url') }}" class="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm outline-none ring-amber-200 transition focus:ring-2">
                @error('image_url')
                    <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
                @enderror
            </div>
